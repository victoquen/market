...
$aColumns = array('id', 'nombre', 'operacion', 'transaccion', 'descripcion', 'fecha', 'monto');
 ...
 */
$sQuery = "
		SELECT a.id as id, b.nombre as nombre, a.operacion as operacion, a.transaccion as transaccion, a.descripcion as descripcion, a.fecha as fecha, a.monto as monto
		FROM   banco_movimiento a
		INNER JOIN banco b ON a.id_banco = b.id_banco
		WHERE (a.borrado = 0) AND (a.id_banco='$banco') 
                $sWhere
		$sOrder
		$sLimit
	";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        if ($aColumns[$i] == "id") {
            ...
            //$sOutput .= ($aRow[ $aColumns[$i] ]=="id_empleado") ?
            //'"-",' :
            //'"'.str_replace('"', '\"', $aRow[ $aColumns[$i] ]).'",';
            $sOutput .= '"' . str_replace('"', '\"', $aRow[$aColumns[$i]]) . '",';
        } else {

            if ($aColumns[$i] == "operacion") {

                $serie_aux = $aRow[$aColumns[$i]];
                if($serie_aux == 1){
                    $sOutput .= '"' . str_replace('"', '\"', "INGRESO") . '",';
                }else{
                    $sOutput .= '"' . str_replace('"', '\"', "EGRESO") . '",';
                }
            }else{
                /* General output */
                $sOutput .= '"' . str_replace('"', '\"', $aRow[$aColumns[$i]]) . '",';
